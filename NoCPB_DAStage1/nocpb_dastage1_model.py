...
class nocpb_dastage1_model(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('loss'):
            ...
            self.g_loss = self.target_gloss + self.lbd * self.supervised_loss
    ...
    def train(self):
        ...
        for itr in range(1, self.total_iteration + 1):
            ...
            _training_accuracy, _g_loss, _d_loss = self.sess.run(
                [self.accuracy_source, self.g_loss, self.d_loss], feed_dict=feed_dict_eval)
            ...
            if itr % self.itr_epoch == 0:
                ...
                log = "Epoch: [%d], Training Accuracy: [%.4f], G Loss: [%.4f], D Loss: [%.4f], Time: [%s]" % (
                    _current_eps, source_training_acc, g_loss, d_loss, time.ctime(time.time()))
